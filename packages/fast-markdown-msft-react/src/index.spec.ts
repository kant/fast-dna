...
describe("Convert markdown to HTML", function(): void {
    ...
    test("should convert a markdown link to a fluent web hyperlink", function(): void {
    ...
    test("should convert a markdown ordered list to a fluent web list", function(): void {
    ...
    test("should convert a markdown heading to a fluent web heading", function(): void {
        const markdown: string = "# Hello world";
        expect(md.render(markdown).replace(newlineRegex, "")).toBe(
            "<Typography id=\"hello-world\" tag=\"h1\" typeLevel={3}>Hello world</Typography>"
        );
    });
